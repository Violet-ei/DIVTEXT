...
def load_data(dataset=None):
    ...
    # fix test: create fake label column if missing
...
class Bert_dataset(Dataset):
    def __init__(self, df):
        ...
        # 从预训练模型（'bert-base-uncased'）加载对应的 BERT 分词器（tokenizer）
        # BERT Tokenizer 是将自然语言文本转换为 BERT 模型能理解的数字ID序列的工具
    ...
    def __getitem__(self, index):
        ...
        # sample: 一条训练样本的完整表示
        # input_ids: BERT 词汇表中的 token ID 序列
        # attention_mask: 指示哪些位置是“真实内容”，即不是填充padding的部分
        # token_type_ids: 告诉模型“这是第几个句子”，单句分类任务中，全为0
        # target: 句子的分类标签
        ...
        for i in trange(len(word_freq)):
            ...
            if word in word2idx:
                if word not in word_hash:
                    ...
                    if args.mapping_strategy == "aggressive":
                        sim_dist_list = np.dot(embeddings[word2idx[word]], embedding_list.T)
                        min_max_dist = max(sim_dist_list) - min(sim_dist_list)
                        min_dist = min(sim_dist_list)
                        new_sim_dist_list = [(x-min_dist)/min_max_dist for x in sim_dist_list]
                        tmp = [np.exp(eps*x/2) for x in new_sim_dist_list]
...
def generate_new_sents_s1(df,sim_word_dict,p_dict,save_stop_words,type="train"):
    ...
    total_words = cnt
    unchanged_words = raw_cnt  # 包括停用词 + OOV + 替换后抽中原词
    actually_changed = total_words - unchanged_words
    replace_rate = actually_changed / total_words if total_words > 0 else 0
    print(f"\n[Substitution Stats] Total words: {total_words}, "
          f"Changed words: {actually_changed}, "
          f"Effective substitution rate: {replace_rate:.2%}\n")
